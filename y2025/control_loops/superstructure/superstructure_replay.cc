...
int main(int argc, char **argv) {
  ...
  reader.RemapLoggedChannel("/superstructure",
                            "y2025rol_loops.superstructure.Status");
  reader.RemapLoggedChannel("/superstructure",
                            "y2025rol_loops.superstructure.Output");
  ...
  roborio->OnStartup([roborio]() {
    roborio->AlwaysStart<y2025::control_loops::superstructure::Superstructure>(
        "superstructure");
  });
  ...
  print_loop->MakeWatcher(
      "/superstructure",
      [&](const y2025::control_loops::superstructure::Status &status) {
        if (status.estopped()) {
          LOG(ERROR) << "Estopped";
        }
      });

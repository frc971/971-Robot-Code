...
ABSL_FLAG(std::string, config, "y2024_bot3/aos_config.json",
          "Name of the config file to replay using.");
ABSL_FLAG(std::string, constants_path, "y2024_bot3/constants/constants.json",
...
int main(int argc, char **argv) {
  ...
  reader.RemapLoggedChannel("/imu/constants", "y2024_bot3.Constants");
  reader.RemapLoggedChannel("/roborio/constants", "y2024_bot3.Constants");
  ...
  imu->OnStartup([imu, &reader, &output_folder]() {
    y2024_bot3::SendSimulationConstants(
    ...
    auto constants = imu->AlwaysStart<
        frc971::constants::ConstantsFetcher<y2024_bot3::Constants>>(
        "constants_fetcher");
